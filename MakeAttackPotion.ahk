...
InitPicLab(){
  FindText().PicLib("|<EyeOfNewt>*123$16.zzzzzzVzwvzbzyDzs3zUDy0zs3zkDzVzzzzzzzzzzzy",1)
  FindText().PicLib("|<GuamPotion>*95$21.z1zzwDzzVzzwDzzVzzsDzz3zzs1zy0DzU1zw0Dz00zk07y00zU03w00TU03y00Dk07zU1zzUTzzzzU",1)
...
TakeOutItems()
{
  if (ok := FindText(X, Y, 260, 30 - WinTitleOffset, 750, 785 + WinTitleOffset, 0, 0, FindText().PicLib("EyeOfNewt"))) {
  ...
  Else{
    OutputDebug, "找不到 EyeOfNewt "
  }

  if (ok := FindText(X, Y, 260, 30 - WinTitleOffset, 750, 785 + WinTitleOffset, 0, 0, FindText().PicLib("GuamPotion"), 0)) { ;0 使用上一次搜索"piedishB"的截图来调用 FindText
...
MakePieShell()
{
  ...
  if (ok := FindText(X, Y, 1020, 660 - WinTitleOffset, 1280, 960 + WinTitleOffset, 0, 0, FindText().PicLib("EyeOfNewt"))) {
  ...
  if (ok := FindText(X, Y, 1020, 660 - WinTitleOffset, 1280, 960 + WinTitleOffset, 0, 0, FindText().PicLib("GuamPotion"), 0)) { ;0 使用上一次搜索"pieDishI"的截图来调用 FindText
    ; 找到 "pastryDough" 图像后要进行的操作
    Random, randIdx, 1, ok.Length()
    X := ok[randIdx].x
    Y := ok[randIdx].y
...
DepositInventory(){
  if (FindText(X, Y, 260, 30 - WinTitleOffset, 750, 785 + WinTitleOffset, 0, 0, FindText().PicLib("depositInventory")))
  {
    ...
    ;检查是否还有库存，没有则退出循环
    if ((ok := FindText(X, Y, 260, 30 - WinTitleOffset, 750, 785 + WinTitleOffset, 0, 0, FindText().PicLib("EyeOfNewt")) == 0)){
      OutputDebug, "库存不足： EyeOfNewt "
      ExitApp
    }
    if((ok2 := FindText(X, Y, 260, 30 - WinTitleOffset, 750, 785 + WinTitleOffset, 0, 0, FindText().PicLib("GuamPotion")) == 0)){
      OutputDebug, "库存不足： GuamPotion "
      ExitApp
    }
    LoopCount ++
