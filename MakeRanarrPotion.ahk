...
InitPicLab(){
  ...
  FindText().PicLib("|<RanarrWeed>*37$30.03xzy03vk20Tnk21y3U23y3U47w004Dw008Tw008DzU083zk0E0zU0E1XU0E3k00U7k00UDk00UDs00ETw03Uzw0w0Tzz0000000U",1)
...
TakeOutItems()
{
  ...
  if (ok := FindText(X, Y, 260, 30 - WinTitleOffset, 750, 785 + WinTitleOffset, 0, 0, FindText().PicLib("RanarrWeed"), 0)) { ;0 使用上一次搜索"piedishB"的截图来调用 FindText
...
MakePieShell()
{
  ...
  if (ok := FindText(X, Y, 1020, 660 - WinTitleOffset, 1280, 960 + WinTitleOffset, 0, 0, FindText().PicLib("RanarrWeed"), 0)) { ;0 使用上一次搜索"pieDishI"的截图来调用 FindText
    ; 找到 "pastryDough" 图像后要进行的操作
    Random, randIdx, 1, ok.Length()
    X := ok[randIdx].x
    Y := ok[randIdx].y
...
DepositInventory(){
  if (FindText(X, Y, 260, 30 - WinTitleOffset, 750, 785 + WinTitleOffset, 0, 0, FindText().PicLib("depositInventory")))
  {
    ...
    if((ok2 := FindText(X, Y, 260, 30 - WinTitleOffset, 750, 785 + WinTitleOffset, 0, 0, FindText().PicLib("RanarrWeed")) == 0)){
      OutputDebug, "库存不足： RanarrWeed"
      ExitApp
    }
    LoopCount ++
